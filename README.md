...
남은 분량 기준 재수립 (plan revision 2, 2026-09-22 개정).

| 도서 | 분량 | 속도 | 계획 완료 |
|------|------|------|-----------|
| 고전 유튜브 | 주 1일 전일 녹음 | — | 매주 **화요일** (6회) |
| 성경 스토리텔링 | 132p (잔여) | 15p/일 | 10-14 녹음 완료 · 10-15 전일 편집 |
| 레바논의 시내 | 197p (잔여) | 20p/일 | 10-16 녹음 완료 · 10-19~11-06 편집 |
| 사도행전 | 120p (일정 역산값) | 10p/일 | 11-06 — 레바논 편집일에만 배치 |

기간 **2026-09-23 → 11-06** · 7주 · 녹음 22일 · 전일 점유 7일 · 총 449p · 최대 일부하 35p · 상한 40p

배분 제외일: **9/24, 9/25, 10/5, 10/9** (근무일이지만 일정 없음)
...
2. **실적 데이터는 읽을 수 없다.** 브라우저 `localStorage` 에만 있다. 진도 분석이나
   ...
   `localStorage.getItem("voxplan.checkins.v2")` 결과를 붙여 주면 된다.
...
저장 키는 `voxplan.checkins.v2` 이고 origin 단위로 격리된다. 저장이 막힌 환경에서는
